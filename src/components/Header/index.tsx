                ...
                >
                  <ul className="block lg:flex">
                    {menuData.map((menuItem, index) => {
                      const isLastSpecial = menuItem.special && index > 0 && menuData[index - 1]?.special;
                      return (
                        <li key={index} className={`group relative ${menuItem.special ? "flex items-center" : "mr-12"} ${isLastSpecial ? "ml-3" : ""}`}>
                          {menuItem.path ? (
                            <Link
                              href={menuItem.path}
                              className={
                                menuItem.special
                                  ? menuItem.filled
                                    ? "inline-flex items-center rounded bg-[#ea7414] px-5 py-1.5 text-sm font-medium text-white transition-all duration-300 hover:bg-[#d66712] hover:shadow-lg hover:shadow-[#ea7414]/20"
